...
import { keywordProjects, keywordProjectItems, contentImports, seoArticles, ragDocuments, ragDocumentTags, tags, tagCategories } from "../drizzle/schema";
...
${ragContext}`
  ...
  seoArticle: router({
    ...
    generateStructure: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const { generateArticleStructure } = await import('./seoArticleGenerator');
        ...
        const structure = await generateArticleStructure(input.theme, ragContext, input.criteria);
        return { structure };
      }),
    ...
    createJob: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        try {
          ...
          console.log('[createJob] Input:', { theme: input.theme, targetWordCount: input.targetWordCount, authorName: input.authorName });
          console.log('[createJob] User ID:', ctx.user.id);
    ...
    rewrite: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        const { rewriteArticle, checkArticleQuality } = await import('./seoArticleGenerator');
        ...
        // リライト後の品質チェック
        // criteriaをqualityCheckから再構築
        const criteria = {
          targetWordCount: input.qualityCheck.wordCount || 5000,
          targetH2Count: input.qualityCheck.h2Count || 8,
          targetH3Count: input.qualityCheck.h3Count || 20,
          targetKeywords: input.qualityCheck.keywordCounts.map((kc: any) => ({
            keyword: kc.keyword,
            minCount: kc.target
          }))
        };
        
        const newQualityCheck = await checkArticleQuality(rewrittenArticle, criteria);
    ...
    polishArticle: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        console.log('[Polish Article] Starting final polish...');
        console.log(`[Polish Article] Article length: ${input.article.length} characters`);
        console.log(`[Polish Article] RAG context length: ${input.ragContext.length} characters`);
        
        const { polishArticleStyle } = await import('./seoArticleGenerator');
        const polishedArticle = await polishArticleStyle(input.article, input.ragContext, input.authorName);
        
        console.log('[Polish Article] Final polish completed');
        console.log(`[Polish Article] Polished article length: ${polishedArticle.length} characters`);
